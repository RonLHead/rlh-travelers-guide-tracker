...
describe('Trips', () => {
  ...
  it("should be able to request a new trip", function () {
    expect(tripsTest.requestNewTrip(8, "2022/12/30", 19, 5, 4)).to.deep.equal({
      "id": 13,
      "userID": 8,
      "destinationID": 4,
      "travelers": 5,
  ...
  it("should return an error message if a requested trip includes over 9 travelers", function () {
    expect(tripsTest.requestNewTrip(7, "2022/03/02", 10, 15, 1)).to.equal("Can only request a trip for 9 travelers or less. Please re-enter with the correct number of travelers.");
  });

  it("should return an error message if a request trip lasts over 1 year (365 days)", function () {
    expect(tripsTest.requestNewTrip(6, "2022/12/25", 400, 5, 5)).to.equal("Cannot request a trip to last more than one year. Please enter a duration for one year or less.");
  });

  it("should return an error message if a request trip starts earlier than today", function () {
    expect(tripsTest.requestNewTrip(1, "2022/01/01", 3, 1, 30)).to.equal("Cannot set the start date to earlier than today. Please select a different start date.");
  });
  ...
  it("should calculate the estimated cost of a new trip request that is pending", function () {
    ...
    expect(tripsTest.pendingTripCost(13)).to.equal(5208.5);
  });
